...
Then will filter/remove the sequence longer than the cutoff value of minimal sequence length in bp. The reads shorter than this cutoff will be output to the result file without compress.
Parameters: outseqf in_seqfile Max_length_cutoff
USAGE: 
`python3 RemoveLongReadsfastq.py OUT_file IN_seq_file Max_length_cutoff`
Example on testing data:
`python3 RemoveLongReadsfastq.py ./testseq.filtered.fasta ./testseq.fasta 5000`
...
usage=""" python3 removeLongReadsfastq.py OUT_file_in_fastq IN_seq_file_in_fastq.gz Max_length_cutoff
          e.g.: python3 removeLongReadsfastq_gz.py testseq.filtered.fastq testseq.fastq.gz 5000
          """
print(usage)
